...
def ne_spiral_arm(x,y,z, gal_param):
    ...
    for j in range(adict['narms']):
        ...
        if len(gd_wa) > 0:  # I think the NE2001 code had a major logic bug in these lines
            gatmp = np.exp(-(smin[gd_wa]/(
            ...
            if np.sum(iclosest) > 0:
                ...
                ga[icutz[gd_wa[iclosest]]] = gatmp[iclosest] # Check this indexing
                # Galactocentric radial dependence of arms
                tmp_rr = rr[icutz[gd_wa[iclosest]]]
                lg_rr = tmp_rr > gal_param['Aa']
                ga[icutz[gd_wa[iclosest]]][lg_rr] *= 1./np.cosh((tmp_rr[lg_rr]-gal_param['Aa'])/2.0)
                if np.sum(lg_rr) > 0:
                    pdb.set_trace()


    # arm3 reweighting:
    iarm3 = np.where(whicharm == 3)[0]
    if len(iarm3) > 0:
        th3a=320.
        th3b=390.
        th3b=370.
        ...
        th3b=363.
        fac3min=0.0
        test3 = thxy[iarm3]-th3a
        neg_t3 = test3 < 0
        test3[neg_t3] += 360.
        gd_t3 = (0. <= test3) & (test3 <  (th3b-th3a))
        if np.sum(gd_t3) > 0:
            arg=6.2831853*(thxy[iarm3][gd_t3]-th3a)/(th3b-th3a)
    ...
    # arm2 reweighting:
    iarm2 = np.where(whicharm == 3)[0]
    if len(iarm2) > 0:
        th2a=35.
        th2b=55.
        test2 = thxy[iarm2]-th2a
        fac = 1.
        if False:
        ...
        if np.sum(gd_t2_snd) > 0:
            arg=6.2831853*(thxy[iarm2][gd_t2_snd]-th2a)/(th2b-th2a)
            fac = (1.+fac2min + (1.-fac2min)*np.cos(arg))/2.
            # Update ga
            ga[iarm2][gd_t2_snd] *= fac
            pdb.set_trace()
